...
async function send(textOverride){
  ...
  try{
    ...
    while(true){const {done,value}=await reader.read();if(done)break;buf+=dec.decode(value,{stream:true});const parts=buf.split("\n\n");buf=parts.pop()||"";
      for(const p of parts){if(!p.startsWith("data:"))continue;const d=JSON.parse(p.slice(5));if(d.requestId)state.requestId=d.requestId;if(d.type==="delta"){assistant.content+=d.delta;renderMessages(false)}else if(d.type==="done"){state.settings.limit=d.limit;updateLimit()}else if(d.type==="error"){throw new Error(d.message)}}
    }
    await saveCurrent();
...
async function init(){
  const d=await api("/api/state");Object.assign(state,{settings:d.settings,conversations:d.conversations,memories:d.memories});applyAppearance();renderChats();renderMessages();initRevealAnimations();
