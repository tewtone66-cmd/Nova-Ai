...
app.get("/dev",(req,res)=>res.sendFile(path.resolve("public/dev.html")));
app.get("/dev/",(req,res)=>res.sendFile(path.resolve("public/dev.html")));
app.post("/api/admin/login",(req,res)=>{const {username,password}=req.body||{};if(!ownerCredentialsValid(username,password))return jsonError(res,401,"نام کاربری یا رمز عبور اشتباه است.","OWNER_INVALID_CREDENTIALS");req.session.owner=true;res.json({ok:true});});
app.post("/api/admin/logout",(req,res)=>{if(req.session)req.session.owner=false;res.json({ok:true});});
app.get("/api/admin/dashboard",requireOwner,(req,res)=>{db=load();const users=Array.isArray(db.users)?db.users:[];let conversations=0,memories=0;for(const user of users){const u=getUserData(db,user.id);conversations+=(u.conversations||[]).length;memories+=(u.memories||[]).length;}const providers={openai:Boolean(process.env.OPENAI_API_KEY),gemini:Boolean(process.env.GEMINI_API_KEY),openrouter:Boolean(process.env.OPENROUTER_API_KEY),stability:Boolean(process.env.STABILITY_API_KEY),pixverse:Boolean(process.env.PIXVERSE_API_KEY),runway:Boolean(process.env.RUNWAY_API_KEY)};const recentUsers=users.slice().sort((a,b)=>(b.createdAt||0)-(a.createdAt||0)).slice(0,12).map(u=>({username:u.username,isGuest:Boolean(u.isGuest),createdAt:u.createdAt}));res.json({ok:true,stats:{users:users.length,conversations,memories},providers,recentUsers});});
...
app.use("/api",(req,res,next)=>{if(req.path.startsWith("/auth/")||req.path==="/health"||req.path.startsWith("/admin/"))return next();requireAuth(req,res,next);});
